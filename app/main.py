...
GET  /api/tickets?status=       List tickets newest-first; optional status filter.
                                &untriaged=true returns the triage work queue.
GET  /api/tickets/{id}          Fetch one ticket (404 if missing).
PATCH /api/tickets/{id}         Update status / priority / assignee.
PATCH /api/tickets/{id}/triage  Write AI classification results onto a ticket.
...
def list_tickets(
    ...
    untriaged: bool = Query(
        default=False,
        description="Return only tickets the triage pipeline hasn't processed yet.",
    ),
    ...
    `untriaged=true` is what the automated triage pipeline polls: it returns
    the work queue of tickets with no classification yet, so the pipeline is
    naturally idempotent — once a ticket is triaged it stops being returned.
    """
    clauses: List[str] = []
    ...
    if untriaged:
        clauses.append("triaged_at IS NULL")

    where = f"WHERE {' AND '.join(clauses)}" if clauses else ""

    with db_conn() as conn:
        rows = conn.execute(
            text(f"SELECT * FROM tickets {where} ORDER BY id DESC LIMIT :limit"),
            params,
        ).mappings().fetchall()
...
@app.patch(
    ...
    tags=["triage"],
    summary="Write AI triage results onto a ticket",
)
def triage_ticket(ticket_id: int, triage: TicketTriage):
    """
    Apply automated triage results to a ticket.

    Separate from PATCH /api/tickets/{id} on purpose: that endpoint is for
    humans changing status and assignment, this one is for the pipeline
    writing classification output. Keeping them apart means the audit trail
    stays readable and either side can change without disturbing the other.

    Setting `triaged_at` is what removes the ticket from the untriaged queue,
    so re-running the pipeline will not reprocess it.
    """
    now = _now_iso()
